...
import lh.content.LHSchematics;
import lh.content.blocks.LHEnvironment;
import mindustry.ai.*;
...
public class TerranitePlanetGenerator extends PlanetGenerator{
    public float heightScl = 0.9f, octaves = 8, persistence = 0.7f, heightPow = 3f, heightMult = 1.6f;
    public static float liqThresh = 0.64f, liqScl = 87f;
    public static float airThresh = 0.13f, airScl = 14;

    Block[] terrain = {LHEnvironment.heatPlate, LHEnvironment.rustPlate, LHEnvironment.electroWater, LHEnvironment.rustPlate, LHEnvironment.heatPlate, LHEnvironment.heatPlate, LHEnvironment.heatPlate, LHEnvironment.heatPlate};
    ...
    protected void generate(){
        ...
        for(Tile tile : tiles){
            ...
            if((floor == LHEnvironment.heatPlate) && rand.chance(0.002)){
                int radius = 2;
                for(int x = -radius; x <= radius; x++){
                    for(int y = -radius; y <= radius; y++){
                        Tile other = tiles.get(x + tile.x, y + tile.y);
                        if(other == null || (other.floor() != LHEnvironment.heatPlate && other.floor() != LHEnvironment.heatPlate1) || other.block().solid){
                            continue outer;
                        }
                    }
                }
                ...
                for(var pos : SteamVent.offsets){
                    Tile otherFl = tiles.get(pos.x + tile.x + 1, pos.y + tile.y + 1);
                    otherFl.setFloor(LHEnvironment.plexPlate.asFloor());
                    Tile otherFl2 = tiles.get(pos.x + tile.x + 3, pos.y + tile.y + 3);
                    otherFl2.setFloor(LHEnvironment.plexPlate.asFloor());
                    Tile otherFl3 = tiles.get(pos.x + tile.x + 3, pos.y + tile.y + 1);
                    otherFl3.setFloor(LHEnvironment.plexPlate.asFloor());
                    Tile otherFl4 = tiles.get(pos.x + tile.x + 1, pos.y + tile.y + 3);
                    otherFl4.setFloor(LHEnvironment.plexPlate.asFloor());
                    Tile other = tiles.get(pos.x + tile.x + 2, pos.y + tile.y + 2);
                    other.setBlock(LHEnvironment.heatMetal);
                    Tile ore = tiles.get(pos.x + tile.x + 2, pos.y + tile.y + 2);
                    ore.setOverlay(Blocks.wallOreBeryllium);
                    Tile other2 = tiles.get(tile.x + 1, tile.y + 1);
                    other2.setBlock(Blocks.darkMetal);
                    other2.setOverlay(Blocks.air);
                }
            }
        }
