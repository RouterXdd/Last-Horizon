...
public class TractorBeamWeapon extends Weapon {
    ...
    protected void shoot(Unit unit, WeaponMount mount, float shootX, float shootY, float rotation) {
        ...
        if (damage > 0) {
            target.damageContinuous(damage * state.rules.unitDamage(unit.team));
        }
        ...
        target.impulseNet(Tmp.v1.set(unit).sub(target).limit((force + (1f - target.dst(target) / range()) * scaledForce) * Time.delta));
